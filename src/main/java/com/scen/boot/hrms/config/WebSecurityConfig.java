        ...
        http.authorizeRequests()
                ...
                .failureHandler(new AuthenticationFailureHandler() {
                    ...
                    public void onAuthenticationFailure(HttpServletRequest req,
                        ...
                        ScenResult scenResult = null;
                        if (e instanceof BadCredentialsException ||
                                e instanceof UsernameNotFoundException) {
                            scenResult = ScenResult.build(403,"账户名或者密码输入错误!");
                        } else if (e instanceof LockedException) {
                            scenResult = ScenResult.build(403,"账户被锁定，请联系管理员!");
                        } else if (e instanceof CredentialsExpiredException) {
                            scenResult = ScenResult.build(403,"密码过期，请联系管理员!");
                        } else if (e instanceof AccountExpiredException) {
                            scenResult = ScenResult.build(403,"账户过期，请联系管理员!");
                        } else if (e instanceof DisabledException) {
                            scenResult = ScenResult.build(403,"账户被禁用，请联系管理员!");
                        } else {
                            scenResult = ScenResult.build(403,"登录失败!");
                        }
                        resp.setStatus(401);
                        PrintWriter out = resp.getWriter();
                        out.write(JsonUtils.objectToJson4e(scenResult));
                        out.flush();
                        out.close();
                    }
                })
